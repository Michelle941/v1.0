...
//begin parties clone
	...
	*/
    ...
    public function actionLogin()
    {
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH'] )){
            	return $this->renderAjax($this->isMobile() ? '_loginForm' : '_loginForm');
		//return $this->renderAjax($this->isMobile() ? 'mobile/_loginForm' : '_loginForm');
        }
    ...
    public function actionMembers()
    {
        ...
        $popularMembersList = '';
        foreach($popularMembers as $pm){
            $popularMembersList .= $pm['id'] . ',';
        }
        $popularMembersList = rtrim($popularMembersList, ',');

	        $popularMembers_section2 = User::find()
            ->orderBy('rank ASC, view_count ASC')
            ->andWhere(['status' => 10])
            ->andWhere(['org_user' => null])
            ->andWhere('tag_line != ""')
            ->andWhere('id not in (' . $popularMembersList . ')')
            ->asArray()
            ->all();
        $popularMembersList_section2 = '';
        foreach($popularMembers_section2 as $pm2){
        $popularMembersList_section2 .= $pm2['id'] . ',';
        }
        $popularMembersList_section2 = rtrim($popularMembersList_section2, ',');

        $popularMembers_section3 = User::find()
            ->orderBy('rank ASC, view_count ASC')
            ->andWhere(['status' => 10])
            ->andWhere(['org_user' => null])
            ->andWhere('tag_line = ""')
	...
	*/
	...
	//removed mobile rendering
        return $this->render($this->isMobile() ? 'publicProfile' : 'publicProfile', [
            'user' => $user,
    ...
    public function actionPhoto($id)
    {
        ...
        return $this->renderAjax($isMobile ? '_photo' : '_photo', [
            'photo' => $photo,
    ...
    public function actionJoin()
    {
        if($data = Yii::$app->request->post() ){
            ...
            if($model->save()){
                return $this->renderAjax($this->isMobile() ? 'complete' : 'complete', [
                    'model' => $model,
                ]);
            }
            else {
                return $this->renderAjax($this->isMobile() ? '_joinForm' : '_joinForm');
            }
        }
    }
...
*/
    ...
    public function actionResetPassword($hash = null)
    {
        ...
        return $this->renderAjax($this->isMobile() ? '/site/_forgotPasswordForm' : '/site/_forgotPasswordForm');
    }
...
/*
    ...
    public function actionParty($id, $preview=0)
    {
        ...
        if($party instanceof Party)
        {
            ...
            return $this->render($isMobile ? 'past_party' : 'past_party',
                    [
                        'party' => $party,
                        'photo' => new photo(),
                        'profile' => $profile
        ...
        if(Yii::$app->request->get('ticket'))
        {
            ...
            $sim = new \AuthorizeNetSIM_Form(
                array(
                    ...
                    'x_relay_url'     => "http://941socialclub.com/site/response",
    ...
    public function actionUploadToParty($id){
        ...
        if($photo->load(Yii::$app->request->post())){
            ...
            if($photo->save()){
                ...
                return $this->renderAjax($isMobile ? '_photo_new' : '_photo', [
                    'photo' => $photo,
                    'new' => 1,
                    'likes' => Likes::getLikes($photo->id),
                    'userId' => Yii::$app->user->getId(),
    ...
    public function actionInstagram()
    {
        ...
        $instagram = new Instagram(array(
            'apiKey'      => '0e83756babc441ca81b9e79071fb72b4',
            'apiSecret'   => 'de7b8638d19342e99b05a981e9f03ef0',
            'apiCallback' => 'http://941socialclub.com/site/instagram' // must point to success.php
        ));
	...
	*/
        ...
        $partyId = isset($photo->party_id) ? $photo->party_id : 0;
        $partyPhoto = Photo::find()
            ->where(['party_id' => $partyId, 'load_user_id' => $userId])
            ->andWhere('(deleted_party =0 or deleted_party is NULL)')
            ->one();
        if(!$partyPhoto){
            if($party2profile = Party2profile::find()->where(['party_id' => $partyId, 'user_id' => $userId])->one())
            {
